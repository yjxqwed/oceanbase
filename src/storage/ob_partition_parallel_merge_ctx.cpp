    ...
    const int64_t parallel_idx, ObExtStoreRange& merge_range, ObIAllocator& allocator)
{
  ...
  if (!is_valid()) {
    ...
    switch (parallel_type_) {
      case PARALLEL_MAJOR:
        if (GCONF.__major_merge_split_new_way) {
...
int ObParallelMergeCtx::get_ranges_by_base_sstable(common::ObIArray<common::ObStoreRange>& ranges)
{
  ...
  ObSSTable *base_sstable = first_sstable_;
  int64_t row_cnt = base_sstable->get_total_row_count();
  if (row_cnt <= 0) {
    ...
    const common::ObIArray<blocksstable::MacroBlockId>& macro_block_ids = base_sstable->get_macro_block_ids();
    ...
    if (OB_UNLIKELY(macro_block_cnt <= 0)) {
      ...
      int64_t num_rows_per_range = GCONF.major_merge_minimum_row_count_per_task;
      common::ObStoreRowkey rowkey;
      common::ObStoreRange range;
      range.set_end_key(common::ObStoreRowkey::MIN_STORE_ROWKEY);
      range.set_left_open();
      range.set_right_closed();
      blocksstable::ObFullMacroBlockMeta blk_meta;
      // generate ranges
      int64_t count = 0;
      for (int64_t i = 0; OB_SUCC(ret) && i < macro_block_cnt; i++) {
        const blocksstable::MacroBlockId &blk_id = macro_block_ids.at(i);
        if (OB_FAIL(base_sstable->get_meta(blk_id, blk_meta))) {
          ...
          count += blk_meta.meta_->row_count_;
          if (count >= num_rows_per_range) {
            count = 0;
            rowkey.assign(blk_meta.meta_->endkey_, blk_meta.meta_->rowkey_column_number_);
            if (
      ...
      if (OB_SUCC(ret)) {
        if (count > 0) {
          if (OB_FAIL(range.get_end_key().deep_copy(range.get_start_key(), allocator_))) {
            STORAGE_LOG(WARN, "Failed to deep copy rowkey.", K(ret));
          } else {
            range.set_end_key(common::ObStoreRowkey::MAX_STORE_ROWKEY);
          ...
          ret = OB_ERR_UNEXPECTED;
          STORAGE_LOG(WARN, "Unexpected empty ranges.", K(ret));
        } else {
          ranges.at(ranges.count() - 1).set_end_key(common::ObStoreRowkey::MAX_STORE_ROWKEY);
...
static int get_ranges_by_iteration(
    common::ObIArray<common::ObStoreRange>& ranges, int64_t& row_cnt,
    ObIStoreRowIterator& iter, common::ObIAllocator& allocator)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_UNLIKELY(merge_ctx.tables_handle_.get_count() <= 1)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_SUCC(ret)) {
        ...
        } else if (OB_FAIL(get_ranges_by_iteration(ranges, row_cnt, mpl_scan_mrg, allocator_))) {
...
int ObParallelMergeCtx::prepare_range_array_for_parallel_major_merge(ObSSTableMergeCtx& merge_ctx)
{
  ...
  } else if (merge_ctx.is_full_merge_) {
